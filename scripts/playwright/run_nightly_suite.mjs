...
const steps = [
  ...
  {
    label: "Follow-up scenario: multi-file feature",
    kind: "node",
    script: "scripts/playwright/run_followup_scenario.mjs",
    args: ["--scenario", "multi-file-feature"],
    retries: 1,
    enabled: Boolean(process.env.OPENAI_API_KEY?.trim()),
    skipReason: "OPENAI_API_KEY is not configured.",
  },
  {
    label: "Multi-ticket project flow",
    kind: "node",
    script: "scripts/playwright/run_multi_ticket_flow.mjs",
    retries: 1,
    enabled: Boolean(process.env.OPENAI_API_KEY?.trim()),
    skipReason: "OPENAI_API_KEY is not configured.",
  },
  {
    ...
    kind: "node",
    script: "scripts/playwright/run_self_learning_e2e.mjs",
    retries: 1,
    enabled: Boolean(process.env.OPENAI_API_KEY?.trim()),
    skipReason: "OPENAI_API_KEY is not configured.",
  },
  {
    label: "Skills & hooks management",
    kind: "node",
    script: "scripts/playwright/run_skills_hooks_e2e.mjs",
    retries: 0,
    enabled: true,
  },
  {
    label: "Command center deep test",
    kind: "node",
    script: "scripts/playwright/run_command_center_deep_test.mjs",
    retries: 1,
    enabled: Boolean(process.env.OPENAI_API_KEY?.trim()),
    skipReason: "OPENAI_API_KEY is not configured.",
  },
